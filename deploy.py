...
def deploy_ssh() -> None:
    ...
    # Проверяем, что /opt/templatus смонтирован в контейнер Caddy
    ensure_mount_cmd = """\
if ! docker inspect safescan-caddy --format='{{ json .Mounts }}' | grep -q '/opt/templatus'; then
  echo 'Mount /opt/templatus not found, recreating container...';
